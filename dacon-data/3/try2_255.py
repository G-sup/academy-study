...
from tensorflow.keras.layers import Dropout,Dense,Conv2D,MaxPooling2D,BatchNormalization,Flatten,LeakyReLU
...
model.add(Conv2D(64, (3,3),padding='same', strides=2))
model.add(LeakyReLU(alpha = 0.1))
model.add(Conv2D(96, (3,3), strides=2,activation='tanh'))
...
model.compile(loss='binary_crossentropy', optimizer=Adam(lr=0.01,epsilon=None),metrics=['acc'])
...
model.fit(x_train,y_train,verbose=1,batch_size=32,epochs=1 ,validation_split= 0.2, callbacks=[es,lr])
