...
def build_model(drop=0.5, optimizer='adam', activation='relu',activation1='relu'):
    inputs = Input(shape=(255,255,1), name='inputs')
    x = Conv2D(32, (2,2),padding='same', strides=1,activation=activation)(inputs)
    x = MaxPooling2D(pool_size=2)(x)
    x = BatchNormalization()(x)
    x = Conv2D(32, (2,2),padding='same', strides=2,activation=activation)(x)
    x = MaxPooling2D(pool_size=2)(x)
    # x = BatchNormalization()(x)
    x = Conv2D(64, (2,2),padding='same', strides=2,activation=activation)(x)
    x = MaxPooling2D(pool_size=2)(x)
    x = BatchNormalization()(x)
    x = Conv2D(128, (2,2), strides=2,padding='same',activation=activation1)(x)
...
model.fit(x_train,y_train,verbose=1, callbacks=[es,lr])#,epochs=1000)
