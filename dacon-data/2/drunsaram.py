...
train = pd.read_csv('./dacon-data/2/train.csv')
test = pd.read_csv('./dacon-data/2/test.csv')
...
for train_index, valid_index in skf.split(train2, t_d):
    ...
    model = modeling()
    mc = ModelCheckpoint('./dacon-data/2/dacon-data/0202_3_best_mc.h5', save_best_only=True, verbose=1)
...
sub
sub.to_csv('./dacon-data/2/0202_5_mnist.csv',index=False)