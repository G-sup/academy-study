...
import pandas as pd
from tensorflow.keras.layers import Dense,Flatten,Conv2D,MaxPooling2D,BatchNormalization, Embedding, Activation,UpSampling2D
...
# np.save("C:/data/lotte/npy/x_D.npy", arr=X)
# np.save("C:/data/lotte/npy/y_D.npy", arr=y)
...
# np.save('C:/data/lotte/npy/pred_x_D.npy', arr=img1)


x = np.load("C:/data/lotte/npy/x_D.npy",allow_pickle=True).reshape(-1,256,256,1)
y = np.load("C:/data/lotte/npy/y_D.npy",allow_pickle=True)

x_pred = np.load('C:/data/lotte/npy/pred_x_D.npy',allow_pickle=True).reshape(-1,256,256,1)
...
autoencoder.add(Conv2D(64,3,padding='same',input_shape=(256,256,1),activation='relu'))
...
model = Sequential()
model.add(Conv2D(32,2,1,input_shape = (256,256,1),padding='same',kernel_initializer='he_normal'))
...
sub.to_csv('C:/data/lotte/LPD_competition/auto1.csv',index=False)