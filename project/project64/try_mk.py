...
generator.add(Dense(512*4*4, input_dim=100, activation='relu'))
...
def gan_train(epoch, batch_size, saving_interval):
  ...
  # 데이터 불러오기
  x_train = np.load('C:/Users/Admin/Desktop/project_64_nf.npy')
  ...
  for i in range(epoch):
        ...
        # 소스코드만 첨부합니다. 만들어진 이미지들은 gan_images 폴더에 저장됩니다.
          if i % saving_interval == 0:
              ...
              gan.save_weights('ganc_mk64.h5')
              fig, axs = plt.subplots(5, 5)
              count = 0
              # img = image.array_to_img(gen_imgs[count, :, :, 0], scale=False)
              # img.save(os.path.join("C:/Users/Admin/Desktop/anime/ganimages/", 'generated_nf128_' + str(i) + '.png'))    

              for j in range(5):
                  for k in range(5):
                      axs[j, k].imshow(gen_imgs[count, :, :, :])
                      axs[j, k].axis('off')
                      count += 1
              fig.savefig("C:/Users/Admin/Desktop/anime/ganimages/gan_modu_64_%d.png" % i)
