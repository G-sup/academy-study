...
aaa = Dense(5, activation='relu')(input1) # input 레이어의 꽁지에 둔다
aaa = Dense(3)(aaa) # 다음레이어의 인풋이 dence1 이니까 뒤에 둔다
aaa = Dense(4)(aaa) 
outputs = Dense(2)(aaa) # output까지 나오게 구성
model = Model(inputs = input1, outputs = outputs) # 마지막에 모델형 선언
model.summary()
# Dense의 이름을 동일하게 해도 자동으로 이름을 바꿔준다.
