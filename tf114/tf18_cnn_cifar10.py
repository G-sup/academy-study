...
x = tf.compat.v1.placeholder(dtype='float32', shape=[None,32,32,3])
y = tf.compat.v1.placeholder(dtype='float32', shape=[None,10])

# 2

# L1.
w1 =  tf.compat.v1.get_variable("w1",shape=[4,4,3,128])
...
# L5
w5 = tf.compat.v1.get_variable('w5', shape = [L4.shape[1]*L4.shape[2]*L4.shape[3], 128],initializer=tf.compat.v1.keras.initializers.VarianceScaling)
b5 = tf.compat.v1.Variable(tf.random.normal([128]), name='b5')
L5 = tf.nn.relu(tf.matmul(L_flat, w5) + b5)
L5 = tf.nn.dropout(L5, rate=0.2)
print(L5)

# L6
w6 = tf.compat.v1.get_variable('w6', shape = [128, 64],initializer=tf.compat.v1.keras.initializers.VarianceScaling)
b6 = tf.compat.v1.Variable(tf.random.normal([64]), name='b6')
L6 = tf.nn.relu(tf.matmul(L5, w6) + b6)
L6 = tf.nn.dropout(L6, rate=0.2)
print(L6)

# hypothesis
w7 = tf.compat.v1.get_variable('w7', shape = [64, 10],initializer=tf.compat.v1.keras.initializers.VarianceScaling)
